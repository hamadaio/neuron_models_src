...
extern Memb_func* memb_func;
 ...
 "setdata_kvm96", _hoc_setdata,
 "rates_kvm96", _hoc_rates,
 "states_kvm96", _hoc_states,
 "trates_kvm96", _hoc_trates,
 0, 0
};
...
#define qa qa_kvm96
 double qa = 9;
#define q10 q10_kvm96
 double q10 = 2.3;
#define tha tha_kvm96
 double tha = 25;
#define tadj tadj_kvm96
 double tadj = 0;
#define temp temp_kvm96
 double temp = 23;
#define usetable usetable_kvm96
 double usetable = 1;
#define vmax vmax_kvm96
 double vmax = 100;
#define vmin vmin_kvm96
 double vmin = -120;
 ...
 "Ra_kvm96", &Ra_kvm96,
 "Rb_kvm96", &Rb_kvm96,
 "temp_kvm96", &temp_kvm96,
 "q10_kvm96", &q10_kvm96,
 "vmin_kvm96", &vmin_kvm96,
 "vmax_kvm96", &vmax_kvm96,
 "tadj_kvm96", &tadj_kvm96,
 "usetable_kvm96", &usetable_kvm96,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 3);
 	...
 	ivoc_help("help ?1 kvm96 /Users/Hamada/Desktop/Branco_2010/x86_64/inwardrect.mod\n");
...
static int  rates (  double _lv ) {
   b = Ra * ( _lv - tha ) / ( 1.0 - exp ( - ( _lv - tha ) / qa ) ) ;
   a = - Rb * ( _lv - tha ) / ( 1.0 - exp ( ( _lv - tha ) / qa ) ) ;
   ntau = 1.0 / ( a + b ) ;
   ninf = a * ntau ;
    return 0; }
...
static int _ode_count(int _type){ hoc_execerror("kvm96", "cannot be used with CVODE"); return 0;}
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 70 in file inwardrect.mod:\n        SOLVE states\n"); nrn_complain(_p); abort_run(error);}
